...
export default function TodoItem({
  ...
  return (
    <li
      className={`min-w-0 rounded-2xl border bg-white px-3 py-3 shadow-sm dark:bg-slate-800 sm:px-5 sm:py-4 ${
        overdue
      ...
      <div className="flex min-w-0 items-start gap-2 sm:gap-4">
            ...
            isCancelada
              ...
              : isConcluida
                ...
                : isEmAndamento
                  ...
                  : 'border-slate-300 bg-white hover:border-blue-400 dark:border-slate-600 dark:bg-slate-900 dark:hover:border-blue-400'
          }`}
        >
          {isConcluida && (
            <svg
        ...
        <div className="min-w-0 flex-1 text-left">
          ...
          <div className="mt-1 flex flex-wrap gap-1">
            ...
            {recorrente && (
              <span className="rounded-full bg-cyan-100 px-1.5 py-0.5 text-[10px] font-medium text-cyan-700 dark:bg-cyan-900/40 dark:text-cyan-300 sm:px-2 sm:text-xs">
            ...
            {overdue && (
              <span className="rounded-full bg-red-100 px-1.5 py-0.5 text-[10px] font-medium text-red-700 dark:bg-red-900/40 dark:text-red-300 sm:px-2 sm:text-xs">
                Vencida
              </span>
            )}
            {dueToday && (
              <span className="rounded-full bg-amber-100 px-1.5 py-0.5 text-[10px] font-medium text-amber-800 dark:bg-amber-900/40 dark:text-amber-300 sm:px-2 sm:text-xs">
                Vence hoje
              </span>
            )}
          </div>
              ...
              {!anexoLoading && !anexoError && anexoUrl && isImageAnexo && (
                ...
                >
                  <img
                    ...
                    onError={retryAnexo}
                    className="max-h-24 rounded-lg border border-slate-200 object-contain dark:border-slate-700"
                  />
                </button>
              )}
              {!anexoLoading && !anexoError && anexoUrl && !isImageAnexo && (
                <button
                  type="button"
                  onClick={() => void openAnexoInNewTab()}
                  className="inline-flex min-w-0 items-center gap-1.5 text-xs font-medium text-blue-600 hover:text-blue-700 dark:text-blue-400 dark:hover:text-blue-300 sm:text-sm"
                >
          ...
          {total > 0 && (
            <div className="mt-2">
              ...
              {checklistOpen && (
                <div className="mt-2 rounded-xl border border-slate-100 bg-slate-50/50 p-3 dark:border-slate-700 dark:bg-slate-900/40">
                  <SubtarefaList
                    mode="readonly"
        ...
        <div className="relative shrink-0" ref={menuRef}>
          <button
            ...
            className="rounded-lg p-1.5 text-slate-400 hover:bg-slate-100 hover:text-slate-600 dark:text-slate-500 dark:hover:bg-slate-600 dark:hover:text-slate-300"
          >
            <DotsVerticalIcon />
          </button>
